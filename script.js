...
<code>https://example.com/landing-page123AB</code>
  ...
  const jobTypeInput = form.querySelector('select[name="job_type"]');
  function updateInfoIconVisibility() {
    if (jobTypeInput && jobTypeInput.value === "links_and_utm") {
  ...
  if (jobTypeInput) {
    jobTypeInput.addEventListener('change', updateInfoIconVisibility);
...
buInput.addEventListener('change', checkDirtyAndUpdateSaveBtn);
      ...
      utmContentGroup.className = "field-group utm-content-group"; // add extra class for styling
      
      // Create a wrapper for the input and icon
...
// Info popup element (create BEFORE appending)
const infoPopup = document.createElement("div");
...
// Add input and icon to wrapper, THEN popup
utmContentWrapper.appendChild(utmContentInput);
utmContentWrapper.appendChild(infoBtn);      // <--- BUTTON
utmContentWrapper.appendChild(infoPopup);    // <--- POPUP (immediate sibling)

// Add to group and form as before
...
form.appendChild(utmContentGroup);
      ...
      function checkDirtyAndUpdateSaveBtn() {
        ...
        // Update file list status to reflect dirty/saved state
        const filenameInput = form.querySelector("input[name='filename']");
     ...
     form.querySelectorAll('input, select, textarea').forEach(el => {
      ...
      // File input change: update filename and clear saved state ONLY if a new file is selected
      fileInput.addEventListener("change", function() {
        ...
        // If no file was selected (user cancelled), do nothing: don't update filename, don't clear saved state, don't run dirty check.
      });
